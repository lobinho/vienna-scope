...
/** Low-Level I/O Implementation in impl_gpio, impl_i2c (not yet working) and impl_gpio **/
...
#ifndef DEBUG_JTAG_I2C
	fprintf(stderr, "xsvftool-gpio using Lib(X)SVF (http://www.clifford.at/libxsvf/).\n");
#else
	fprintf(stderr, "xsvftool-i2c using Lib(X)SVF (http://www.clifford.at/libxsvf/).\n");
#endif
...
static void help()
{
	copyleft();
	fprintf(stderr, "\n");
	fprintf(stderr, "Usage: %s [ -v ... ] [ -L | -B ] { -s svf-file | -x xsvf-file | -c } ...\n", progname);
	fprintf(stderr, "\n");
...
int main(int argc, char **argv)
{
	...
	progname = argc >= 1 ? argv[0] : "xsvftool";
	while ((opt = getopt(argc, argv, "vLBs:c")) != -1)
	{
		switch (opt)
		{
		case 'v':
		...
		case 's':
			gotaction = 1;
			if (u.verbose)
				fprintf(stderr, "Playing SVF file `%s'.\n", optarg);
			if (!strcmp(optarg, "-"))
				u.f = stdin;
			else
				u.f = fopen(optarg, "rb");
			if (u.f == NULL) {
				fprintf(stderr, "Can't open SVF file `%s': %s\n", optarg, strerror(errno));
				rc = 1;
				break;
			}
			if (libxsvf_play(&h, LIBXSVF_MODE_SVF) < 0) {
				fprintf(stderr, "Error while playing SVF file `%s'.\n", optarg);
				rc = 1;
			}
			if (strcmp(optarg, "-"))
				fclose(u.f);
